...
<main class="enhancement">
  ...
  <section class="section">
    ...
    <img src="Pictures/Enhancements2/login.png" alt="Case-Insensitive Login">
    <p>
      The login system allows users to enter their username in any letter case and still authenticate successfully, reducing friction and user error without weakening password security.
    </p>
    <div class="enhancement-details">
      <p><strong>Beyond requirements:</strong> Enhances usability by normalizing username comparison at query-time while preserving the originally cased username in storage and sessions for display consistency. Security is maintained by keeping passwords case-sensitive and hashed using password_hash().</p>
      <p><strong>Implementation steps:</strong> Use prepared statements with LOWER(username) = LOWER(?) in queries (see login_process.php for admin and membership checks); keep the stored username unchanged; apply password_verify() directly to the submitted password (no lowercasing) to retain full entropy; escape any echoed username with htmlspecialchars(); ensure updates to usernames in admin tools continue to use consistent case-insensitive matching logic; optionally add an index on LOWER(username) (or a generated column) for scalability.</p>
    </div>
    <div class="button-group">
